...
//			 Writer theWriter = new Writer();
//				theWriter.printArfflemmaVector(corpus, "data/output/lemma.arff", IxTheo_Annotation.KDB);
	...
	private static IxTheoCorpus createGermanCorpus(IxTheoCorpus c) {
		...
		for (IxTheoRecord rec : c.getFileList()) {
			if (rec.getLanguage().contains("ger")) {
				cGer.getFileList().add(rec);
			}
		}
		return cGer;
	}
...
//		reader = new MARC4JProcessor("data/GesamtTiteldaten-post-pipeline-160612.xml");
//		reader = new MARC4JProcessor("data/testCorpus.xml");
		...
		System.out.println("num files in entire corpus: " + corpus.getFileList().size());
